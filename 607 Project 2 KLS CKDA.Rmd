...
data1.2<-
  ...
  arrange(Country) %>%
  separate(Date, c("Month", "Day", "Year")) #separate date into month, day, year for analysis 
```

## Dataset 1 - Analysis
# Analysis Question 1 - are we collecting enough fees vs what the company is charging  

```{r}
#Analysis 1 - Ratio of tax charged 

# Convert data1.2 Price of Carrier and Shipping Fees Collected from chr to num
data1.2[5:6] <- lapply(data1.2[5:6], as.numeric)

analysis1<- data1.2 %>%
            mutate(Ratio = `Price of Carrier`/`Shipping Fees Collected`)

analysis1
```

# Analysis Question 2 - What is the shipping cost per month?
...
analysis2<- data1.2 %>%
            group_by(Month) %>% 
            summarise(Average = mean(`Price of Carrier`, na.rm = TRUE))
analysis2
```

# Analysis Question 3 - What is the shipping cost per month by country?
...
#Convert Average hourly earnings in dollars and Average weekly earnings in dollars columns from chr to num 
data2final[3:4] <- lapply(data2final[3:4], as.numeric)
```


## Dataset 2 - Analysis
# Analysis Question 1 - What is the average earnings across industries by date

```{r}
#Analysis 1 - Average earnings by industry

analysis4<- data2final %>%
           group_by(Industry) %>% 
           summarise(Average = mean(`Average hourly earnings in dollars`, na.rm = TRUE)) 
...
analysis5<- data2final %>%
           group_by(Industry) %>% 
           summarise(Average = mean(`Average weekly earnings in dollars`, na.rm = TRUE)) 
